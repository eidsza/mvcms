...
//    $this->db_password = DB_PASS;
    ...
    private function _get_user_resources(){
      ...
      $resources = $this->db->SELECT("select * from mvcms_acl_user_resources");
      return $resources;
    ...
    private function _get_user_permissions(){
     ...
     $perms = $this->db->SELECT("select * from mvcms_acl_user_permissions");
     return $perms;
    ...
    public function _get_role_permissions($role_id){
     ...
     $perm_roles = $this->db->SELECT("select * from mvcms_acl_user_permissions WHERE role = :role", array("role"=>$role_id));
      
     $this->ecms3_role_permitions = $perm_roles;
     return $perm_roles;
        ...
        public function get_permisions($role, $resource){
        $perms = array("can_access"=>0, "can_add"=>0, "can_modify"=>0, "can_modify_own"=>0, "can_delete"=>0, "can_delete_own"=>0);
        
        $perms["can_access"]    =$this->cmsacl->isAllowed($role, $resource, 'has_access')? 1 : 0;
        $perms["can_add"]       =$this->cmsacl->isAllowed($role, $resource, 'add')? 1 : 0;
        $perms["can_modify"]    =$this->cmsacl->isAllowed($role, $resource, 'modify')? 1 : 0;
        $perms["can_modify"] +=$perms["can_modify_own"]=$this->cmsacl->isAllowed($role, $resource, 'modify_own')? 1 : 0;
        $perms["can_delete"]    =$this->cmsacl->isAllowed($role, $resource, 'delete')? 1 : 0;
        $perms["can_delete"] += $perms["can_delete_own"]=$this->cmsacl->isAllowed($role, $resource, 'delete_own')? 1 : 0;
