...
function smarty_prefilter_form($source,&$smarty)
{
        ...
        if (preg_match('|{formprocessor.*?}|is',$source))
        {
                ...
                include_once(SMARTY_DIR . $smarty->plugins_dir[0] . '/shared.form.php');
                global $smarty_form;
                ...
                $source = preg_replace_callback('/{formprocessor *?(.*?)}(.*?){\/formprocessor}/is',array(&$smarty_form,'compiler_formprocessor'),$source);
        }
        return $source;
}
