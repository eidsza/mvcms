...
<ul class="sf-menu template-menu" id="custom_menu_1" name="Główne menu">
        ...
        {assign var='dph' value=0}
              {section name=row loop=$custom_menu_1}
                   ...
                   <li class="item">                   
                   <a {if (isset($vars.pid) && $vars.pid == $custom_menu_1[row].fk_page_id)} class='active' 
                      {elseif (!isset($vars.pid) && $smarty.section.row.index==0 ) }  class='active' {/if} href="{$BASEURL}page/show/{$custom_menu_1[row].fk_page_id}" title="{$custom_menu_1[row].title}" alt="{$custom_menu_1[row].title}">
