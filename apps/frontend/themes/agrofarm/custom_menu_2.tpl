<ul class="sf-menu template-menu" id="custom_menu_2" name="Główne menu">
        {assign var='mdepth' value=1}
        {assign var='dph' value=0}
              {section name=row loop=$custom_menu_2}
                   {if ($custom_menu_2[row].depth < $mdepth)} </li> {/if}
                   {if ($custom_menu_2[row].depth > $mdepth)} 
                      {assign var='dph' value = $custom_menu_2[row].depth-$mdepth} {'<ul>'|str_repeat:$dph}
                   {/if}
