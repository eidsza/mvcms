...
<div style="display:none;" id="dialog-confirm" title={$vars['lang']['category.dialog_confirm_title']}">
    ...
    <p class="pageoptions">
        ...
        <img src="{$vars.view_url}images/icons/Paste32.png" class="systemicon" alt="Dodaj nową kategorię" title="Dodaj nową kategorię" />
        </a>
     ...
     <tr>
      ...
      {if  !empty($vars.categories) }   
          ...
          {foreach from=$vars.categories item=p}
              {assign var='row_id' value=categorytable_|cat:$p['id']}
             ...
             <td><h2>{$p['name']}</h2>
                ...
                <thead><tr>
                    ...
                    {assign var=pageex value=$vars.category_details[$p['id']]}
                                       
                    {foreach from=$pageex item=details}
                            <tr>
                            <td><img src="{$vars.view_url}images/icons/Flag16.png" /></td>
                            <td>{$details['cat_ln']}</td>
                            <td>{$details['category_title']}</td>
                            <td></td>
                            <td>{$details['date_publish']}</td>
                            <td>{$details['date_update']}</td>
                            <td><a href="{$vars.baseurl}admin/news/category/setVisible/{if $details['is_visible']}0{else}1{/if}/{$details['id']}"><img src="{$vars.view_url}images/icons/{if $details['is_visible']}{if $details['date_publish'] > $smarty.now}star_half_full16.png" title="Ukryj (jeszcze nie opublikowana)"{else}Star_full16.png " title="Ukryj"{/if}{else}Star_empty16.png" title="Pokaż"{/if} /></a></td>
                            <td><a title="Edytuj artykuł" href="{$vars.baseurl}admin/news/category/edit/{$p['id']}/{$details['id']}/{$details['cat_ln']}"><img src="{$vars.view_url}images/icons/File_edit16.png" /></a></td>
                            </tr>
                     {/foreach}
                 </tbody>
                    </table>
                ...
                <td valign="top"><a title="Usuń wszystkie kategorie!" href="{$vars.baseurl}admin/news/category/delete/{$p['id']}"><img src="{$vars.view_url}images/icons/Folder_delete16.png" /></a></td>
                </tr>  
